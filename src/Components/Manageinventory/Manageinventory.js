...
const Manageinventory = () => {
  ...
  const [url,setUrl] =useState(`https://young-beach-37066.herokuapp.com/products?limit=${limit}&page=${curpage}`)
  useEffect(() => {
        ...
        {showmpro && (
          <div className=" min-h-screen">
            ...
            <div className="px-5">
                {
                  [...Array(pages).keys()]
                  .map(num=>(<button className="btn ml-1" key={num} onClick={()=>{setUrl(`https://young-beach-37066.herokuapp.com/products?limit=${limit}&page=${num}`)}}>{num}</button>))
                }
            </div>
          </div>
        )}
